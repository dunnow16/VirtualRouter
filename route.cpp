...
int main(int argc, char** argv) {
  ...
  int lastSentArpReq = 0;  // last packet sent was ARP request
  uint8_t lastSentArpReqPacket[5000];
...
// - if have dif hdrs at the same mem distance from ether hdr it shouldn't work
                        ...
                        { 
                            ...
                            if ((it=net2hop.find(daddr & 0xffffff00)) != net2hop.end() ) {  // 16 bit netlength
                                ...
                                hopaddrnet = (uint32_t)htonl(hopaddr);
                                printf("    hop addr: %s\n", inet_ntoa(*(struct in_addr*)&hopaddrnet));
                            } else if (lastSentArpReq){
                                cout << "ARP Timeout, Host Unreachable" << endl;
                                ...
                                icmphdr_reply->code = (uint8_t)1;  //8 (host unreachable)
                                ...
                                //ethernet header
                                struct ether_header* sentehdr = (struct ether_header*) lastSentArpReqPacket;
                                struct ether_header* sentiphdr = (struct ether_header*) lastSentArpReqPacket;
                                ehdr_reply->ether_type = sentehdr->ether_type;
                                memcpy(ehdr_reply->ether_shost, sentehdr->ether_dhost, ETH_ALEN);
                                memcpy(ehdr_reply->ether_dhost, sentehdr->ether_shost, ETH_ALEN);
                                ...
                                memcpy(iphdr_reply, sentiphdr, sizeof(struct iphdr));
                                ...
                                memcpy(icmphdr_reply+sizeof(struct ouricmp), sentiphdr, sizeof(struct iphdr));
                                ...
                                cout << "sending ICMP destination (network) unreachable message" << endl;
                            ...
                            // Save packet in case of host unreachable error
                            memcpy(lastSentArpReqPacket, pckt, sizeof(*packet));
                            send(portNum, packet, sizeof(struct ether_header) + 
                                sizeof(struct ether_arp), 0);
                            lastSentArpReq = 1;
                            //-------------
